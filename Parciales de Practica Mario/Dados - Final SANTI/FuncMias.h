...
//	void etiquetar_regiones_no_solapadas();
...
//			cout << "Porcentaje de la botella: " << (np_empty *100)/np_full << " % \n" << endl;
	...
	inline Mat DibujarHoughHorizontal(Mat img, double nLineas){
	...
	inline Mat DibujarHoughVertical(Mat img, double nLineas){
		//Canny para detectar los bordes de la imagen
		//cvtColor para cambiar la imagen de escala de grises a BGR y poder mostrar los resultados.
		// nLineas = cantidad minima de lineas a detectar
		Mat dst, cdst;
...
//	inline void crecer(int i,int j){
//		tam++;
//		etiq.at<unsigned char>(i,j) = etiqueta;
//		for(int k = -1;k<2;k++){
//			for(int m = -1;m<2;m++){
//				int ni=i+k;
//				int nj=j+m;
//				if( img.at<unsigned char>(ni,nj)!=0 and etiq.at<unsigned char>(ni,nj)==0 and ni!=nj){
//					crecer(ni,nj);
//				}
//			}
//		}
...
//	inline void etiquetar_regiones_no_solapadas(){
////		Mat etiq,img;
//		int etiqueta;
//		int tam;
//		
...
//				if( (int)img.at<uchar>(i,j)!= 0 && (int)etiq.at<uchar>(i,j)==0 ){
//					tam =0;
//					crecer(i,j);
//					if (tam>5000){
//						cout<<"Tamaño: "<<tam<<endl;
//						monedas++;
//						if(tam>23000)eur2++;else
//						if(tam>21000)cent50++;else
//						if(tam>19000)eur1++;else
//						if(tam>17500)cent20++;else
//						if(tam>16000)cent5++;else
//						if(tam>13800)cent10++;else
//						if(tam>12000)cent2++;else cent1++;
//											
//					}
//					etiqueta++;
//				}
