...
*/
    ...
    cv::Mat filtro_movimiento(size_t rows, size_t cols, double a,double b){
        cv::Mat filter[2];
        filter[0]=cv::Mat::ones(rows, cols, CV_32F);filter[1]=cv::Mat::ones(rows, cols, CV_32F);
        cv::Mat result= cv::Mat::ones(rows, cols, CV_32F);
        for(size_t K=0; K<rows; ++K)
            for(size_t L=0; L<cols; ++L){
            float ad=(1/(M_PI*(K*a+L*b)))*sin(M_PI*(K*a+L*b));
                filter[0].at<float>(K,L)=ad;
                filter[1].at<float>(K,L)=exp(M_PI*(K*a+L*b));
            }
        cv::merge(filter, 2, result);
        return result;
        }
